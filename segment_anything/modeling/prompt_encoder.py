...
class PromptEncoder(nn.Cell):
    ...
    def _embed_points(
        ...
        if pad:
            ...
            padding_label = -ops.ones((labels.shape[0], 1))
    ...
    def construct(
        ...
        sparse_embeddings = Tensor(shape=(bs, 1, self.embed_dim), dtype=mindspore.float32)
        if points is not None:
            coords, labels = points
            point_embeddings = self._embed_points(coords, labels, pad=(boxes is None))
            sparse_embeddings = ops.cat([sparse_embeddings, point_embeddings], axis=1)
        if boxes is not None:
            box_embeddings = self._embed_boxes(boxes)
            sparse_embeddings = ops.cat([sparse_embeddings, box_embeddings], axis=1)
...
class PositionEmbeddingRandom(nn.Cell):
    ...
    def __init__(self, num_pos_feats: int = 64, scale: Optional[float] = None) -> None:
        ...
        if scale is None or scale <= 0.0:
            scale = 1.0
        self.positional_encoding_gaussian_matrix = scale * ops.randn((2, num_pos_feats))
    ...
    def construct(self, size: Tuple[int, int]) -> Tensor:
        ...
        pe = self._pe_encoding(ops.stack([x_embed, y_embed], dim=-1))
    ...
    def construct_with_coords(
        ...
        return self._pe_encoding(coords.to(ms.float))  # B x N x C
